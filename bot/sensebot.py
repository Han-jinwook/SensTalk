...
class SenseBotRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if parsed.path == "/sync":
        ...
        elif parsed.path == "/self-update":
            # 1-클릭 안티그래비티 스타일 자체 엔진 업데이트 엔드포인트
            candidate_urls = [
                data.get("url"),
                f"https://raw.githubusercontent.com/Han-jinwook/SensTalk/main/bot/sensebot.py?t={int(time.time())}",
                f"https://sensetalk.netlify.app/sensebot.py?t={int(time.time())}"
            ]
            ...
            try:
                ...
                for target_url in candidate_urls:
                    try:
                        print(f"[*] 자체 업데이트(Self-Update) 신호 수신 -> 최신 소스 다운로드 시도: {target_url}")
                        req = urllib.request.Request(
                            target_url,
                            headers={"User-Agent": "SensTalk-AutoUpdater"}
                        )
                        with urllib.request.urlopen(req, timeout=12) as resp:
                            if resp.status == 200:
                                downloaded = resp.read().decode("utf-8")
                                if len(downloaded) >= 5000 and "SENSEBOT_VERSION" in downloaded:
                                    new_code = downloaded
                                    print(f"[✓] 다운로드 성공: {target_url} ({len(new_code)} bytes)")
                                    break
                    except Exception as e:
                        last_download_err = e
                        print(f"[!] 다운로드 실패 ({target_url}): {e}")

                if not new_code:
                    raise Exception(f"모든 다운로드 소스 접속 실패: {last_download_err}")

                # 파이썬 구문 유효성 검사 (Syntax Check)
                compile(new_code, "<updated_sensebot>", "exec")

                # 최신 버전 정보 추출
                ver_match = re.search(r'SENSEBOT_VERSION\s*=\s*["\']([^"\']+)["\']', new_code)
                new_version = ver_match.group(1) if ver_match else "latest"

                # 현재 실행 중인 파일 백업 및 덮어쓰기
                current_file = os.path.abspath(__file__)
                backup_file = current_file + ".bak"

                try:
                    with open(current_file, "r", encoding="utf-8") as cur_f:
                        old_code = cur_f.read()
                    with open(backup_file, "w", encoding="utf-8") as bak_f:
                        bak_f.write(old_code)
                except Exception as bak_err:
                    print(f"[*] 백업 파일 생성 알림: {bak_err}")

                with open(current_file, "w", encoding="utf-8") as f:
                    f.write(new_code)

                print(f"[✓] sensebot.py 파일 갱신 성공! (v{SENSEBOT_VERSION} -> v{new_version})")
                ...
                res = {
                    "status": "success",
                    "old_version": SENSEBOT_VERSION,
                    "new_version": new_version,
                    "message": f"v{new_version} 다운로드 및 파일 교체 완료! 새 엔진으로 자동 재실행합니다."
                }
                self.wfile.write(json.dumps(res, ensure_ascii=False).encode("utf-8"))
                ...
                threading.Thread(target=restart_worker, daemon=False).start()
                return

            except Exception as e:
                print(f"[!] 자체 업데이트 오류: {e}")
